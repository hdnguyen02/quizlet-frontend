<template>
    <div class="mt-4 max-w-3xl w-full">
        ...
        <button @click="isStart=true" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Bắt đầu</button>
    </div>
    <div v-show="isStart" class="flip-card w-full h-[340px]">
     
        <div v-if="curentCard" class="flip-card-inner  shadow-3xl bg-white rounded-lg text-2xl" :class="{'is-flip': isFlip,'x': isX}">
      ...
      <div v-show="isStart" class="mt-1">
        ...
        <div v-if="isShowAnswer" class="flex gap-x-4 justify-center">
          ...
          <button @click="handlerChooseQuality" data-response="hard" class="w-28 bg-transparent hover:bg-red-500 text-red-700 font-semibold hover:text-white py-2 px-4 border border-red-500 hover:border-transparent rounded">
            Khó
          </button>
        </div>
          <div class="flex flex-col items-center justify-center">
          <span class="text-sm">{{computedDay.medium}} day</span>
          <button @click="handlerChooseQuality" data-response="medium" class="w-28 bg-transparent hover:bg-orange-500 text-orange-700 font-semibold hover:text-white py-2 px-4 border border-orange-500 hover:border-transparent rounded">
            Trung bình
          </button>
        </div>
        <div class="flex flex-col items-center justify-center">
          <span class="text-sm">{{computedDay.good}} day</span>
          <button @click="handlerChooseQuality" data-response="good" class="w-28 bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded">
            Tốt
          </button>
        </div>
        <div class="flex flex-col items-center justify-center">
          <span class="text-sm">{{computedDay.veryGood}} day</span>
          <button @click="handlerChooseQuality" data-response="veryGood" class="w-28 bg-transparent hover:bg-green-500 text-green-700 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded">
            Rất tốt
          </button>
        </div>
        </div>
...
<script>
    ...
    export default { 
        ...
        methods: { 
            getCards(){ 
                this.$axios.get(`api/v1/card/study?deskId=${this.idDesk}`)
                .then(apiResponse => {
            ...
            handlerChooseQuality(event ) {
                ...
                if (this.isStudyCard) {
                  ...
                  if (this.indexStudyCard == this.studyCards.length) {
                  ...
                  if (this.indexReviewCard == this.reviewCards.length) {
        ...
        watch: {
          isComplete(newValue) {
            if(newValue == true) {
              // hoàn thành việc học 
              console.log(this.learnedCards)
              console.log(this.reviewedCards)
            }
          }
        }
    }
